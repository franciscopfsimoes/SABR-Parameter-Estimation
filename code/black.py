...
def D1(
    S, K, T, r, vol
):  # Returns value of function d1 in Black-Scholes' pricing formula
...
def Vol(
    S, K, T, p, r, iscall
):  # Returns Black-Scholes' implied volatility using Newton-Raphson's method
    ...
    while epsilon > tol:  # Newton-Raphson's method
        ...
        price = (
            Price(S, K, T, vol, r, iscall) - p
        )  # Difference between Black-Scholes's price of the contract considering volatility v and the market price of the contract

        vega = Vega(
            S, K, T, r, vol, iscall
        )  # Black-Scholes' greek Vega (\partial price / \partial v)

        vol = vol - price / vega  # Newton-Raphson's step

        epsilon = math.fabs(
            (vol - origVol) / origVol
        )  # Newton-Raphson's epsilon error after step
